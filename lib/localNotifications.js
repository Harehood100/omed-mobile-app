...
// Fires when the user taps a notification — including opening the app from a fully killed
// state. This is the most reliable way to report a trigger for a killed app, since tapping
// actually launches real JS as part of resuming/starting the app (unlike a local notification
// firing on its own, which can't run JS while the app is dead). Also checks for the response
// that may have launched the app in the first place (getLastNotificationResponseAsync).
export function registerReminderResponseListener(onReminderTriggered) {
